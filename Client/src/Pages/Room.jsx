...
const {user}=useAuth();
 ...
 const onAllUsers = (users) => {
  setRemoteUsers(users);
  users.forEach((user) => createPeerConnection(user.id));
};

const onUserJoined = ({ id, name }) => {
...
const onUserDisconnected = (id) => {
  ...
  <div key={user.id} className={getVideoTileClass(user.id)}>
    {remoteVideoStatus[user.id] === false && (
      <div className="video-avatar">
        <VideoOff size={isMobile ? 24 : 32} />
      </div>
    )}
    <video
      ...
      autoPlay
      playsInline
      className={getVideoClass(user.id)}
      style={{ display: remoteVideoStatus[user.id] !== false ? "block" : "none" }}
    />
    <div className="video-label">
      <span className="video-name">{user.name}</span>
